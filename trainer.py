...
from input_pipe import InputPipe, ModelMode, FakeSplitter, train_features, infer_features
...
def train(name, hparams, multi_gpu=False, n_models=1, train_completeness_threshold=0.01,
          ...
          max_steps=None, save_from_step=None, do_eval=True, **args):
    ...
    with tf.device("/cpu:0"):
        ...
        splitter = FakeSplitter(train_features(inp), 3, seed=seed, test_sampling=eval_sampling)
    ...
    def create_model(scope, index, prefix, seed):
        ...
        with tf.variable_scope('input') as inp_scope:
            with tf.device("/cpu:0"):
                ...
                pipe = InputPipe(datadir, inp, features=split.train_set, mode=ModelMode.TRAIN, 
                                 batch_size=batch_size, n_epoch=None, verbose=verbose,
                                 train_completeness_threshold=train_completeness_threshold,
                                 train_window=train_window, train_skip_first=hparams.train_skip_first, 
                                 rand_seed=seed)
                inp_scope.reuse_variables()
                if forward_split:
                    forward_eval_pipe = InputPipe(datadir, inp, features=split.test_set, mode=ModelMode.EVAL, 
                                                  batch_size=eval_batch_size, n_epoch=None, verbose=verbose, 
                                                  runs_in_burst=eval_batches,
                                                  train_completeness_threshold=0.01, train_window=train_window, 
                                                  rand_seed=seed)
                else:
...
def predict(checkpoints, hparams, datadir="data", verbose=False, n_models=1,
            target_model=0, asgd=False, seed=1, batch_size=50):
    with tf.variable_scope('input') as inp_scope:
        with tf.device("/cpu:0"):
            inp = VarFeeder.read_vars(os.path.join(datadir, "vars"))
            pipe = InputPipe(datadir, inp, infer_features(inp), mode=ModelMode.PREDICT, batch_size=batch_size,
                             n_epoch=1, verbose=verbose,
                             train_completeness_threshold=0.01, train_window=hparams.train_window)
        ...
        for checkpoint in checkpoints:
            ...
            while True:
                try:
                    ...
                    utf_names = pname
                    pred_df = pd.DataFrame(index=utf_names, data=np.expm1(pred) - 1)
